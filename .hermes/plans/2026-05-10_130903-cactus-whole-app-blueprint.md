...
- Vault should use the shared top bar, one obvious search, one `Add Data` CTA, neutral colors, and column-level filters like Excel/Airtable. Empty Vault should say only `No data yet` plus `Add Data`. The Add Data page should make the pipeline visually obvious: sources fill the Vault. Use `Connect a Data Source`, `API partner feed`, and `Confirm before saving`; avoid `Connect one account`, dead `Need API...` links, and `Review before Vault`.
