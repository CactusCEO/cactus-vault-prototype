...
- Spaces should show who is working in or has access to a Space with small profile avatars in History rows/cards and in the Space header/share controls. New Space creation should be simple: name, starting context (selected Vault rows, folder, source, workflow, blank), update mode (frozen/latest/auto-updating), and collaborators — not a heavy builder.
